...
def train(audio_model, train_loader, test_loader, args):
    ...
    if args.dataset == 'audioset':
        ...
        loss_fn = nn.BCEWithLogitsLoss()
        warmup = False
    elif args.dataset == 'microphone':
        print('scheduler for microphone is used')
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, list(range(5,26)), gamma=0.85)
        main_metrics = 'acc'
        loss_fn = nn.CrossEntropyLoss()
        warmup = False
...
def validate(audio_model, val_loader, args, epoch):
    ...
    with torch.no_grad():
        ...
        #print(target)
        #print(audio_output)
        target_ = np.argmax(target.cpu().numpy(), axis=-1)
        audio_output_ = np.argmax(audio_output.cpu().numpy(), axis=-1)
        #print(target_)
        #print(audio_output_)
        cm = confusion_matrix(target_, audio_output_)
        #/print(cm)
        #plt.figure(figsize=(10,10))
        #plot_confusion_matrix(cm, ['C','D','M'])
        disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['C','D','M'])
        disp.plot(cmap=plt.cm.Blues)
        plt.title('7m Confusion Matrix')
        plt.show()
        #plt.savefig('cm_9m_a.png')
        plt.savefig('cm_7m_5.pdf')
        '''
